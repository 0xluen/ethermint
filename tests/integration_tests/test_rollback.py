...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:daily_4568-1-node{i}":
            ini[section].update(
                {
...
def custom_ethermint(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-dailyd.nix",
    ]
    print(*cmd)
    broken_binary = (
        Path(subprocess.check_output(cmd).strip().decode()) / "bin/dailyd"
    )
    print(broken_binary)
...
def test_rollback(custom_ethermint):
    ...
    print("stop node1")
    supervisorctl(
        custom_ethermint.base_dir / "../tasks.ini", "stop", "daily_4568-1-node1"
    )
    ...
    update_node_cmd(custom_ethermint.base_dir, "dailyd", 1)

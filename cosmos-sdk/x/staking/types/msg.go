...
func (msg MsgCreateValidator) ValidateBasic() error  {
	...
	maxCommission, maxCommissionErr := sdk.NewDecFromStr("1.0")
	if maxCommissionErr != nil {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "invalid max commission")
	}

	minCommission, minCommissionErr := sdk.NewDecFromStr("0.9")
	if minCommissionErr != nil {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "invalid max commission")
	}

	if msg.Commission.Rate.LTE(maxCommission) && msg.Commission.Rate.GTE(minCommission) {
		return nil
	}

	return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "commission rate must be between 0.9 and 1.0")
}
